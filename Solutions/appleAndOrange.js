...
function countApplesAndOranges(s, t, a, b, apples, oranges) {
    ...
    for(let i=0; i<apples.length; i++){
        let posicao = apples[i] + a;
        if (posicao >= s && posicao <=t ) contMacas ++;
    }
    
    for(let i=0; i<oranges.length; i++){
        let posicao = oranges[i] + b;
        if (posicao >= s && posicao <=t ) contLaranjas ++;
    }
    console.log(contMacas);
    console.log(contLaranjas);
}
